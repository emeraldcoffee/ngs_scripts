...
while getopts "s:S:i:a:h" opt; do
    case $opt in
        s) 
            ...
            sample_1_1=${sample_1_1}
            sample_1_2=${sample_1_2}
            ;;
        S)
            sample_2="$OPTARG"
            echo "Sample 2: $sample_2"
            sample_2_1=${sample_2_1}
            sample_2_2=${sample_2_2}
            ;;
        i)
            index="$OPTARG"
...
if compgen -G "${sample_2}/*_R_fastqc.html" > /dev/null; then
    ...
    fastqc \
        -t 1 ${sample_1_1}.fq.gz &
    fastqc \
        -t 1 ${sample_1_2}.fq.gz &
    fastqc \
        -t 1 ${sample_2_1}.fq.gz &
    fastqc \
        -t 1 ${sample_2_2}.fq.gz &
...
if [[ $REPLY = "yes" ]]; then
    ...
    # FastQC
    fastqc \
        -t 2 ${sample_1_1}_trimmed.fq.gz &
    fastqc \
        -t 2 ${sample_1_2}_trimmed.fq.gz &
    fastqc \
        -t 2 ${sample_2_1}_trimmed.fq.gz &
    fastqc \
        -t 2 ${sample_2_2}_trimmed.fq.gz &
...
STAR \
    --runThreadN 18 \
    --genomeDir ${index} \
    --readFilesIn ${sample_1_1}_trimmed.fq.gz \
...
STAR \
    --runThreadN 18 \
    --genomeDir ${index} \
    --readFilesIn ${sample_1_2}_trimmed.fq.gz \
    --readFilesCommand zcat \
    --outSAMtype BAM SortedByCoordinate \
    --outSAMattributes NH HI AS NM MD \
    --outFilterMultimapNmax 1 \
...
STAR \
    --runThreadN 18 \
    --genomeDir ${index} \
    --readFilesIn ${sample_2_1}_trimmed.fq.gz \
    --readFilesCommand zcat \
    --outSAMtype BAM SortedByCoordinate \
    --outSAMattributes NH HI AS NM MD \
    --outFilterMultimapNmax 1 \
...
STAR \
    --runThreadN 18 \
    --genomeDir ${index} \
    --readFilesIn ${sample_2_2}_trimmed.fq.gz \
    --readFilesCommand zcat \
    --outSAMtype BAM SortedByCoordinate \
    --outSAMattributes NH HI AS NM MD \
    --outFilterMultimapNmax 1 \
