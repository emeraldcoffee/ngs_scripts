...
echo -e "Beginning paired-end RNA sequencing upstream analysis.\n"

# runs FastQC HTML generation for users to check sequencing quality
if compgen -G "${sample_1_1}/${sample_1_1}_F_fastqc.html" > /dev/null; then
    ...
    for ((i=1;i<=num_samples; i++)); do
        ...
        if compgen -G "${!sample1}"/"${!sample1}"_F.fq.gz > /dev/null; then
            mv "${!sample1}"/"${!sample1}"_F.fastq.gz "${!sample1}"/"${!sample1}"_F.fq.gz
            mv "${!sample1}"/"${!sample1}"_R.fastq.gz "${!sample1}"/"${!sample1}"_R.fq.gz
            mv "${!sample2}"/"${!sample2}"_F.fastq.gz "${!sample2}"/"${!sample2}"_F.fq.gz
            mv "${!sample2}"/"${!sample2}"_R.fastq.gz "${!sample2}"/"${!sample2}"_R.fq.gz
        fi
...
# uses fastp to trim off adapters and poly-G tails from sequencing data
if compgen -G "${sample_1_1}/trimmed_${sample_1_1}_F.fq.gz" > /dev/null; then
    ...
    for ((i=1;i<=num_samples; i++)); do
        ...
        fastp \
            ...
            -o trimmed_${sample1}_F.fq.gz \
            -O trimmed_${sample1}_R.fq.gz &
        ...
        fastp \
            ...
            -o trimmed_${sample2}_F.fq.gz \
            -O trimmed_${sample2}_R.fq.gz &
...
if [[ $REPLY = "yes" ]]; then
    for ((i=1;i<=num_samples; i++)); do
        ...
        fastqc -t 4 \
            "${!sample1}"/trimmed_"${!sample1}"_F.fq.gz \
            "${!sample1}"/trimmed_"${!sample1}"_R.fq.gz \
            "${!sample2}"/trimmed_"${!sample2}"_F.fq.gz \
            "${!sample2}"/trimmed_"${!sample2}"_R.fq.gz &
...
if compgen -G "${sample_1_1}/${sample_1_1}_Aligned.sortedByCoord.out.bam" > /dev/null; then
    ...
    for ((i=1;i<=num_samples; i++)); do
        ...
        STAR \
            --runThreadN 18 \
            --genomeDir ${index} \
            --readFilesIn trimmed_${sample1}_F.fq.gz trimmed_${sample1}_R.fq.gz \
        ...
        STAR \
            --runThreadN 18 \
            --genomeDir ${index} \
            --readFilesIn trimmed_${sample2}_F.fq.gz trimmed_${sample2}_R.fq.gz \
            --readFilesCommand zcat \
            --outSAMtype BAM SortedByCoordinate \
            --outSAMattributes NH HI AS NM MD \
            --outFilterMultimapNmax 1 \
